...
Handlebars.registerHelper('formatInput', function(n) {
  if (isNaN(n)) {
    ...
    return numberWithCommas(n) + " Golden Coins";
  }
});
...
var humanoidMonths = ["Jaugmix", "Fersix", "Mathix", "Arsix", "Marfix", "Jupsix", "Jugrix", "Aupfix", "Segrix", "Okgix", "Noyrdix", "Defajix"];
Handlebars.registerHelper('randomHumanoidDate', function() {
  const currentDate = new Date()
  return ordinal(getRandomInt(1,30)) + " of " + getRandom(humanoidMonths) + " " + currentDate.getFullYear() + ".";
});
...
complaintStacker = new Stacker($('body'), 200, 200, -2, 2, 'right', 'bottom');
var complaints= [];
var bigNotes = [];
var smallNotes = [];
var mayhem = {};
...
var numNobleComplaints = getRandomInt(2, 10) * getRandomInt(1, 2);
var numPeasantComplaints = getRandomInt(4, 20) * getRandomInt(1, 5);
var numTownsefolkeComplaints = getRandomInt(3, 15) * getRandomInt(1, 3);

for (var i = 0; i < numNobleComplaints; i++) {
    item = new RoyalGrievance();
    bigNotes.push(royalComplaint(item));
    complaints.push(item);
    description = "Damages to " + item.title + " " + item.name + "'s lands (" + numberWithCommas(item.value) + " Golden Coins)";
...
for (i = 0; i < numPeasantComplaints; i++) {
    ...
    description = "Damages to " + item.name + "'s " + item.type + " (" + numberWithCommas(item.value) + " Golden Coins)";
    complaints.push(item);
    if ((item.type === "crops" && item.value > taxRules.exemptionLimits.crops) ||
        ...
        addToRecord("B1", 0, description, "Exc. for damages to " + item.type +" under "  + numberWithCommas(taxRules.exemptionLimits[item.type]) + " Golden Coins" );      
    }
...
for (i = 0; i < numTownsefolkeComplaints; i++) {
  ...
  complaints.push(item);
  description = "Damages to " + (item.shopName || item.name + "'s property") + " (" + numberWithCommas(item.value) + " Golden Coins)";
  if (item.value > taxRules.exemptionLimits.provincial) {
    mayhem.townsfolk += item.value;
    addToRecord("B2", item.value, description);
  } else {
    addToRecord("B2", 0, description, "Exc. for provincial damages under "  + numberWithCommas(taxRules.exemptionLimits.provincial) + " Golden Coins" );      
  }
...
appraisalStacker = new Stacker($('body'), 120, 20, -2, 2, 'right', 'bottom');
formStacker = new Stacker($('body'), 2, 2, 4, 60);
...
for (var season = 0; season < seasons.length; season++) {
    for (var day = 1; day <= days; day++) {
        ...
        if (schedule.indexOf(scehduleDay) > -1) {
            ...
            ledgerTable.append(ledgerRow({description:"- Golden Coins", amount:" + " + numberWithCommas(amount), balance:hoard.gold}));
            addToRecord("A1", amount, raidtype);
            if (gemSchedule.indexOf(scehduleDay) > -1) {
                var num_gems = getRandomInt(1,125) * getRandomInt(1,125);
				...
				gemPriceValue = num_gems * gemValue;
                ledgerTable.append(ledgerRow({description: "- " + num_gems + gem_plural, amount:numberWithCommas(Math.abs(Math.floor(gemPriceValue))) + " Golden Coins (Gems)"}));
            ...
            if (magicSchedule.indexOf(scehduleDay) > -1) {
                ...
                description = "Magic " + item.item + " (" + numberWithCommas(item.value) + " Golden Coins)";
                ...
                ledgerTable.append(ledgerRow({description:"- Magic " + item.item, amount:numberWithCommas(Math.abs(Math.floor(item.value))) + " Golden Coins (Magic)"}));
            }
            if (artSchedule.indexOf(scehduleDay) > -1) {
                item = new Art();
                hoard.art.push(item);
                description = item.item + " of " + item.subject + " (" + numberWithCommas(item.value) + " Golden Coins" + ")";
                if (item.value >= taxRules.exemptionLimits.art) {
                   income.art += item.value;
                    addToRecord("A4", item.value, description); 
                } else {
                    addToRecord("A4", 0, description, "Exc. for art valued under " +  numberWithCommas(taxRules.exemptionLimits.art) + " Golden Coins"); 
                }
                appraisalStacker.add(artAppraisalReport(item));
                ledgerTable.append(ledgerRow({description:"- " + toTitleCase(item.item) + " of " + item.subject, amount:numberWithCommas(Math.abs(Math.floor(item.value))) + " Golden Coins     (Art)"}));
            }
            if (treasureSchedule.indexOf(scehduleDay) > -1) {
                item = new Treasure();
                hoard.treasures.push(item);
                description = item.adjective + " " + item.item + " (" + numberWithCommas(item.value) + " Golden Coins" + ")";
                if (item.value >= taxRules.exemptionLimits.treasure) {
                   income.other += item.value;
                    addToRecord("A5", item.value, description); 
                } else {
                    addToRecord("A5", 0, description, "Exc. for treasure valued under " +  numberWithCommas(taxRules.exemptionLimits.treasure) + " Golden Coins"); 
                }
                appraisalStacker.add(treasureAppraisalReport(item));
                ledgerTable.append(ledgerRow({description:"- " + toTitleCase(item.adjective) + " " + item.item, amount:numberWithCommas(Math.abs(Math.floor(item.value))) + " Golden Coins (Treasure)"}));
            }
        }
    }
}
    ...
    if (diffs.hasOwnProperty(l)) {
      ...
      if (["a1","a2","a3","a4","a5","b1","b2","b5"].indexOf(l) > -1) {
        if (difference === 0) {
          ...
          result += "You overreported by " + numberWithCommas(difference) + " Golden Coins.";
          over++;
        } else {
          result += "You underreported by " + numberWithCommas(Math.abs(difference)) + " Golden Coins.";
          under++;
        }
        ...
        if (Math.floor(difference) === 0) {
          ...
          result += "You overpaid by " + numberWithCommas(Math.floor(difference)) + " Golden Coins. You won't get that back!";
        } else {
          result += "You underpaid by " + numberWithCommas(Math.abs(Math.floor(difference))) + " Golden Coins.";
        ...
        if (difference === 0) {
          ...
          result += "This line was too high by " + numberWithCommas(difference) + " Golden Coins.";
          matherrors++;
        } else {
          matherrors++;
          result += "This line was too low by " + numberWithCommas(Math.abs(difference)) + " Golden Coins.";
        }
      }
      result+= "</li>";
